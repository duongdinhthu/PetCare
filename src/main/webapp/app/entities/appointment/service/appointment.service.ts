...
type RestOf<T extends IAppointment | NewAppointment> = Omit<T, 'apptTime' | 'createdAt'> & {
...
export class AppointmentService {
  ...
  protected convertDateFromClient<T extends IAppointment | NewAppointment | PartialUpdateAppointment>(appointment: T): RestOf<T> {
    return {
      ...appointment,
      apptTime: appointment.apptTime?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restAppointment: RestAppointment): IAppointment {
    return {
      ...restAppointment,
      apptTime: restAppointment.apptTime ? dayjs(restAppointment.apptTime) : undefined,
